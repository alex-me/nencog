...
can be imported into cnn_cifar.py or scnn_cifar.py

alex    May 2018
        Jun 2018 - extended to scnn_cifar.py
...
- it is better to leave None the items "classes", "n_class", so that can be validated by
  adapting to the current selection of CIFAR-100, as done in cnn_cifar.py or scnn_cifar.py by setup_cifar()
...
nn_arch         = {
    ...
    "features"      : ( 64, 16 ),        # number of convolution features for each layer
...
#        } 
    ...
    "use_dropout"   : ( True, True ),   # use dropout in the fully-connected layers
