...
def setup_nn(funct,         # la funzione che deve essere rappresentata
            dim = 1,        # numero di paramteri della funzione
    ...
    with model:
        ...
        inp_ens = nengo.Ensemble(n_neurons, dimensions=dim, radius=1)
        nengo.Connection(input_node, inp_ens)

        out_ens = nengo.Ensemble(n_neurons, dimensions=1, radius=2)
        conn = nengo.Connection(inp_ens, out_ens, function=lambda x: np.random.random(1), learning_rule_type=nengo.PES(learning_rate))
        ...
        nengo.Connection(out_ens, error)
    ...
    with model:
        global out_ens_probe, inp_ens_probe, input_node_probe, output_node_probe, error_probe, learn_probe
        input_node_probe = nengo.Probe(input_node)
        output_node_probe = nengo.Probe(output_node)
        inp_ens_probe = nengo.Probe(inp_ens, synapse=0.01)
        out_ens_probe = nengo.Probe(out_ens, synapse=0.01)
...
def recall_nn(simulator, xinput, funct):
    ...
    return sim.data[out_ens_probe][int(t)*1000-1] # utlimo segnale

# Plot dei dati di simulazione
def plot_sim(sim):
    plt.figure(figsize=(9, 9))
    plt.subplot(3, 1, 1)
    #dim = len(input_node_probe)
    plt.plot(
        sim.trange(),
    ...
    plt.plot(
        sim.trange(),
        sim.data[out_ens_probe] - sim.data[output_node_probe],
        label = 'Error')
    plt.legend(loc='lower right')
...
def funct( x ):
    ...
    return np.sin(np.pi*x[0])+np.sin(np.pi*x[1])-np.cos(np.pi*x[2])-np.cos(np.pi*x[3])

# Fase di Training della rete
training_time = 240 # tempo in secondi di addestramento (equivale a 400 input/10 al secondo)
build_dset(funct,dim=dimension,size=10000) 
model   = setup_nn(funct,dim=dimension,training=1) # training=1 sta per l'addestramento della rete
...
for i in range(attempts):
    ...
    for j in range(dimension):
        xinput.append(np.random.random()*2-1.0)
    r = recall_nn(sim, xinput, funct)
    v = funct(xinput)
